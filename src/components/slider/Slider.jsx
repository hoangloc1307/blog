...
function Slider({ slides, show }) {
    ...
    const handleControlClick = (direction) => {
        ...
        if (direction === 'next' && firstSlide <= slides.length - show) {
            slidesRef.current.style.transform = `translate(-${slideWidth * firstSlide}px)`;
            setfirstSlide(firstSlide + 1);
        }
        //Click prev button
        if (direction === 'prev' && firstSlide > 1) {
            slidesRef.current.style.transform = `translate(-${slideWidth * (firstSlide - 2)}px)`;
            setfirstSlide(firstSlide - 1);
        }
    };
        ...
        <div className={c.slider}>
            ...
            <div className={c.controls}>
                <div className={c.buttons}>
                    <div ref={prevButtonRef} className={c.prev} onClick={() => handleControlClick('prev')}>
                        <i className="fa-solid fa-chevron-left"></i>
                    </div>
                    <div ref={nextButtonRef} className={c.next} onClick={() => handleControlClick('next')}>
